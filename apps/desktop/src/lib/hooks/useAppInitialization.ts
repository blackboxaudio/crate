...
import type { DiscoveryRelease, UsbDevice } from '$shared/types'
...
import { registerDesktopStoreHooks } from './useStoreHooks'
...
export async function useAppInitialization(config: AppInitConfig): Promise<() => void> {
	...
	// Register desktop-specific hooks before loading stores
	registerDesktopStoreHooks()
