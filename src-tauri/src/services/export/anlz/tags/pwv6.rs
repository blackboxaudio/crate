//! PWV6 (Extended Waveform) tag - high-resolution waveform for .2EX files
//!
//! Contains extended waveform data with 3 bytes per entry.

#![allow(dead_code)]

use binrw::{BinRead, BinWrite};

/// PWV6 tag header size (12 bytes standard + 8 bytes extra)
...
/// PWV6 (Extended Waveform) tag
///
/// High-resolution waveform for .2EX files with 3 bytes per entry.
#[derive(Debug, Clone, BinRead, BinWrite)]
#[brw(big, magic = b"PWV6")]
pub struct ExtWaveform1Tag {
...
impl ExtWaveform1Tag {
    /// Create a new extended waveform tag with placeholder data
    ///
    /// # Arguments
    ...
    /// Create an extended waveform tag from actual data
...
mod tests {
    ...
    #[test]
    fn test_ext_waveform1_magic() {
        let tag = ExtWaveform1Tag::new(60000);
        let mut buf = Cursor::new(Vec::new());
        tag.write(&mut buf).unwrap();
        let data = buf.into_inner();
        assert_eq!(&data[0..4], b"PWV6");
    }

    #[test]
    fn test_ext_waveform1_entries() {
        let tag = ExtWaveform1Tag::new(60000); // 1 minute = 60s * 150 = 9000 entries
        assert_eq!(tag.len_entries, 9000);
        assert_eq!(tag.data.len(), 9000 * 3);
    }
}
