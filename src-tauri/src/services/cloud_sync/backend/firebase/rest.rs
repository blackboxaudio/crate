...
/// Map a non-success Firestore/Storage HTTP response to a generic `CloudSync` error.
/// (CAS-conflict detection is done explicitly at the manifest commit site; blob 404
/// → `CloudSyncBlobNotFound` at the blob site.)
pub(crate) async fn http_error(context: &str, resp: reqwest::Response) -> CrateError {
    let status = resp.status();
    let body = resp.text().await.unwrap_or_default();
    CrateError::CloudSync(format!("{context}: HTTP {status}: {body}"))
}
