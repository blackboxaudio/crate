...
- Added discovery playlists on mobile: create, rename, and delete playlists and folders with drill-in folder navigation on the Playlists tab, tap a playlist to view its releases in a full-screen detail overlay, add releases from the feed or release detail via a long-press context menu or multi-select batch action, remove releases with swipe or context menu, and long-press-drag to reorder releases in a dedicated reorder mode — all backed by a new `reorder_playlist_releases` backend command and bidirectional cloud sync
