...
- Added the mobile release detail screen (artwork, metadata, one-tap open in the source app — Bandcamp, SoundCloud, YouTube, Discogs — inline-editable notes, an assignable tag picker, and the track list) with an iOS-style left-edge swipe-back, plus an integrated preview player — a persistent mini-player bar with progress that expands (tap or swipe up) into a full-screen player with a blurred album-art backdrop, drag-to-dismiss, a scrubber, and previous/next — and, on iOS, a native lock-screen transport (play/pause, previous/next, and scrubbing that keep working while the screen is locked, with auto-resume after call interruptions) powered by a native AVPlayer engine
