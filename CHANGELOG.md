...
- Added a "Date Liked" sort to the mobile release lists (the discovery feed and the playlist, tag, and following views), offered while the Liked filter is set to Only: releases order by their most recently liked track, so a listening session's finds stay together and the newest ones surface first. Likes now record when they happened (unliking clears it, re-liking stamps it fresh), the timestamp syncs across devices and is included in backups, and likes made before this release sort after dated ones

- Made the discovery filters three-way on desktop and mobile: Liked, New, Purchased, and Downloaded each switch between Off, Only, and Not, so a dimension can be excluded as well as required — e.g. Liked = Only plus Purchased = Not lists the releases you want but don't own yet. The states combine, read the same way in the main feed, inside discovery playlists and folders, in the mobile playlist/tag/following views, and in the Purchased view, and the desktop feed now says when the active filters hid every release instead of suggesting you add your first one
...
- Made tagging and playlist filing faster on desktop: the "Add to playlist" and track "Tags" context menus now follow the sidebar's hierarchy (playlist folders and tag categories open as nested sub-menus instead of one flat list), and with discovery tracks selected, clicking a tag in the sidebar applies it to the selection — clicking again removes it, with the same applied / partial indicators releases and library tracks already had. A tag can also be dragged from the sidebar onto a discovery track (or onto a selected track, to tag the whole selection), which previously did nothing. Also fixed tagging a track from its context menu failing with an error

- Made every context menu on desktop and mobile follow one layout: actions on the item first, then filing (Add to Playlist, Tags, Set Color, Follow), then editing and management, then links and sharing (Open in Bandcamp / SoundCloud / YouTube, Search on YouTube, Share, Copy URL), with destructive actions always red and last — so the same item sits in the same place in every menu. "Tags" is now available wherever a track or release has a menu: library tracks and discovery releases on desktop (nested by category, with the same applied / partial check marks), and the release long-press menu, release detail, and now-playing screen on mobile (opening the tag picker, which gained a search field). The desktop library track menu lists "Remove from Playlist" and "Remove from Library" directly instead of behind a "Remove" sub-menu, "Add to Playlist" is shown disabled rather than hidden when there is no playlist to add to, and the mobile playlist picker shows the full folder path while drilling into folders
- Discovery playlists are now track-based: adding a release adds each of its tracks as a member, and a single track can be added from its context menu (desktop) or long-press menu (mobile), so opening a playlist later shows exactly which tracks motivated the entry. Playlist rows still group by release, showing only the member tracks and a "3 of 12 tracks" label when a release is only partly included; tracks can be removed individually; on desktop, tracks inside expanded releases can be selected like rows (click, Cmd-click, Shift-range, spanning releases), dragged onto a discovery playlist in the sidebar, and acted on together from the context menu or the Delete key; existing playlists carry over with identical contents (every release expanded to all of its tracks, order preserved), including playlists restored from older backups or synced from a device on an older build; and playback, previews, offline audio, and cloud sync behave as before
...
- Fixed iOS previews of YouTube and Discogs tracks reporting twice their real length and going silent once scrubbed past the midpoint: YouTube's fragmented audio files carry a full-length duration in their headers that AVFoundation counts a second time, so the cached copy on iOS now has those header fields cleared before it is played
